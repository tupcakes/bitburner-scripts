...
export async function main(ns) {
	...
	while (true) {
		...
		let allservers = JSON.parse(ns.read("serversbyhacklvl.json.txt"));
		let usableservers = [];

		let pservs = ns.getPurchasedServers();
		// add all rootable servers that have ram and we have root on
		for (const server of allservers) {
			if (ns.getServerMaxRam(server.name) > 0 && ns.hasRootAccess(server.name)) {
				usableservers.push(server.name);
			}
		}
